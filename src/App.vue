<template>
  <div id="app">
    <!-- 实际的音频标签  这里绑定的值应该是audio标签可设置的自带属性-->
    <audio id="myaudio" ref="audio" @timeupdate="musicTimeUpdate" @canplay="musicCanPlay" @playing="musicOnPlaying" @ended="musicEnded" @waiting="musicOnWaiting" @pause="musicOnPause" @loadstart="loadStart"></audio>
    <!-- 头部header -->
    <v-header :index="2"></v-header>
    <!-- 侧边栏列表 -->
    <side-bar :info="info"></side-bar>
    <!-- 底部显示的悬浮层 音乐列表 -->
    <music-list></music-list>
    <!-- 底部固定页(播放区) -->
    <bottom-bar></bottom-bar>
    <!-- 歌单详情页 -->
    <song-sheet></song-sheet>
    <!-- 音乐播放详情  显示cd页 -->
    <music-detail></music-detail>
  </div>
</template>

<script>
import header from'./components/header/header.vue'
import sidebar from './components/sidebar/sidebar.vue'
import mymusic from './components/mymusic/mymusic.vue'
import musiclist from './components/musiclist/musiclist.vue'
import bottombar from './components/bottombar/bottombar.vue'
import songsheet from './components/songsheet/songsheet.vue'
import musicdetail from './components/musicdetail/musicdetail.vue'
import axios from 'axios'
import Vueaxios from 'vue-axios'
import store from './store'

let musicLrcIndex = 0
export default {
  name: 'App',
  data(){
    return {
      info: {}
    }
  },
  methods:{
    // 音乐播放时间更新事件
    musicTimeUpdate () {
      store.dispatch({
        type: 'set_CurrentTime',
        time: Math.floor(this.$refs.audio.currentTime)
      })

      //设置歌词内容（以索引的形式显示  大概模拟）
      let musicLrc = store.getters.getCurrentMusic.lyric//获取当前播放的音乐的歌词
      let currentTime = Math.floor(this.$refs.audio.currentTime)//当前进度
      if(musicLrc[musicLrcIndex] === undefined) return
      if(musicLrc.length === 0){//如果没有歌词
        store.commit({
          type: 'setLyricIndex',
          index: -1
        })
        return
      }
      for(let i = 0; i < musicLrc.length; i++){
          if(currentTime >= Number(musicLrc[musicLrcIndex].timeId)) {
            musicLrcIndex += 1
            break
          }else{
            if(musicLrcIndex <= 0){
              musicLrcIndex = 0
            }else{
              musicLrcIndex --
            }
          }
      }
      store.commit({
        type: 'setLyricIndex',
        index: musicLrcIndex
      })
    },
    // 可以播放事件
    musicCanPlay () {
      store.dispatch({
        type: 'set_MusicDuration',
        duration: Math.floor(this.$refs.audio.duration)//储存当前音频的长度
      })
      store.commit({
        type: 'setMusicLoadStart',
        isloadstart: false
      })
    },
    // 音乐处于播放状态
    musicOnPlaying (){

    },
    // 音频播放结束事件
    musicEnded () {
      store.dispatch('play_Ended')  
      //歌词初始化
      musicLrcIndex = 0
      store.commit({
        type: 'setLyricIndex',
        index: 0
      })
    },
    // 音乐处于等待加载状态
    musicOnWaiting () {

    },
    // 音乐处于暂停状态
    musicOnPause () {

    },
    // 音乐加载
    loadStart () {

    }


  },
  components:{
    'v-header':header,
    'side-bar':sidebar,
    'bottom-bar':bottombar,
    'music-list':musiclist,
    'song-sheet':songsheet,
    'music-detail':musicdetail
  },  
  created () {
    console.log(store)
    let LocalAPI = 'static/data.json'
    axios.get(LocalAPI).then((res) => {
      // data.user的信息赋值给info  再通过组件的数据传递传给sideBar
      this.info = res.data.user
      //把所有的音乐数据给vuex的通过audio.js 里面的action 的setMusicAllList传给musicAllList
      store.dispatch('set_MusicAllList',res.data.music)
      //存储所有的信息 包括音乐信息 个人信息
      store.dispatch('set_AllInfo',res.data)
      //设置音乐的地址  初始化 根据vuex的currentIndex(歌曲索引)来决定
      this.$refs.audio.setAttribute('src',store.getters.getCurrentMusic.url)
      //给audio元素存在vuex的state里面  方便以后调用
      store.dispatch('set_AudioElement', this.$refs.audio)
    }, (err) => {
      alert(err)
    })
  }
}
</script>

<style lang="stylus" rel="stylesheet/stylus">
@import "../static/font-icon/style.css"
@import "common/stylus/border-1px/index.styl"
@import "common/stylus/global.styl"
body,html
  margin:0
  padding:0
  box-sizing:border-box
  background:#f0f0f0f0
  user-select:none
  -webkit-tap-highlight-color: transparent

#app
  font-family: 'Avenir', Helvetica, Arial, sans-serif
  -webkit-font-smoothing: antialiased
  -moz-osx-font-smoothing: grayscale
  color: #2c3e50
</style>