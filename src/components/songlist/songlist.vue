<template>
    <div class="songlists">
        <div class="songlist" v-if="getSongSheet.info" v-for="(item,index) in getSongSheet.info" :key=index @click="playIndex(index)">
            <div class="warpper">
                <div class="listIndex">
                    <!-- 歌曲序号与正在播放的图标 -->
...
export default {
    methods:{
		...
		playIndex (index) {
			// 如果是正在播放的就返回空
			if(index === this.$store.getters.getCurrentIndex && this.$store.getters.getMusicSheetList.type === this.$store.getters.getMusicSheetType){
				return
			}
			store.commit({
