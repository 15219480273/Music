    ...
    <div class="menulist">
        <transition name="sideUp">
            <div class="content" ref="content" v-show="showMenu">
                ...
                <type-list v-for="(item,index) in menuList.content" :item="item" :index="index" :key="item.id" :name="item.name" :iconinfo="item.iconinfo" :count="item.count" :bgcolor="item.bgcolor">
                </type-list>    
            </div>
        </transition>
		<transition name="fade">
			<!-- @touchmove.stop.prevent="stopTouch($event)"  应该是阻止冒泡事件 -->
